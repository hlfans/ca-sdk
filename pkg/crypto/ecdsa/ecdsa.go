...
var (
	...
	errUnknownCurve              = fmt.Errorf(`unknown elliptic curve`)
	errUnknownHash               = fmt.Errorf(`unknown hashing algorithm`)
	errUnknownSignatureAlgorithm = fmt.Errorf(`unknown signature algorithm`)

	errInvalidPrivateKey = fmt.Errorf(`invalid private key, expected ECDSA`)
	errInvalidPublicKey  = fmt.Errorf(`invalid public key, expected ECDSA`)
	errInvalidSignature  = fmt.Errorf(`invalid ECDSA signature`)
)
...
func New(opts map[string]string) (*Suite, error) {
	...
	if cs.sigAlgorithm, err = getSignatureAlgorithm(options.SignatureAlgorithm); err != nil {
		return nil, fmt.Errorf(`signature algorithm: %w`, err)
	}
	return cs, nil
}
